...
def main(_config, _run):
    ...
    if configs["save_log"]:
        writer_strings = ["stdout", "tensorboard", "sacred", "csv", "wandb"]
        writer_strings = ["stdout", "tensorboard", "sacred", "csv"]
        folder = configs["base_folder"] / str(_run._id)
        logger = Logger(
            writer_strings=writer_strings,
            folder=folder,
            # project="ddcfr-ppo-experiments",  # 必须：您在 wandb 上的项目名称
            # config=configs,                   # 推荐：记录本次运行的超参数
            # name=f"run_{_run._id}",           # 可选：为本次运行命名
            # group="PPO Training"              # 可选：将多次运行分组
        )
    else:
        logger = Logger(
