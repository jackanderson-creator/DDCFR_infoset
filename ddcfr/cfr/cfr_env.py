...
class CFREnv(gym.Env):
    ...
    def reset(self):
        ...
        #在状态中添加每个信息集的平均遗憾值
        state = [(iters, conv_frac,np.mean(list(i.regrets.values()))) for i in self.solver.states.values()]
    ...
    def _unscale_action(self, action):
        ...
        tau = [self.tau_list[i] for i in tau]
